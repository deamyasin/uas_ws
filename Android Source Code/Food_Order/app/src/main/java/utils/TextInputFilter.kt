...
class TextInputFilter : InputFilter {
    ...
    override fun filter(
        ...
        return if (source is SpannableStringBuilder) {
            checkFilterFirst(source, start, end)
        } else {
            return checkFilterSecond(source, start, end)
        }

    }

    private fun checkFilterFirst(
        ...
        for (i in end - 1 downTo start) {
            ...
            when (selectedFilter) {
                Filter.NOT_ALLOW_SPECIAL_CHARS, Filter.NOT_ALLOW_SPACE_AND_SPECIAL_CHARS -> {
                    if (!Character.isLetterOrDigit(currentChar)) {
                        source.delete(i, i + 1)
                    }
                }
                Filter.NOT_ALLOW_SPACE -> {
                    if (Character.isWhitespace(currentChar)) {
                        source.delete(i, i + 1)
                    }
                }
                Filter.NOT_ALLOW_ARABIC_NUMBER -> {
