...
public class CircleLayoutManager extends RecyclerView.LayoutManager {
    ...
    private static int mIntervalAngle = 30;// The default interval angle between each items
    private static float mDistanceRatio = 10f; // Finger swipe distance divide item rotate angle

    //Flags of scroll dirction
    private static int mScrollLeft = 1;
    private static int mScrollRight = 2;
    ...
    private void layoutItems(RecyclerView.Recycler recycler, RecyclerView.State state) {
        layoutItems(recycler, state, mScrollRight);
    }
    ...
    private void layoutItems(RecyclerView.Recycler recycler,
        ...
        for (int i = 0; i < getItemCount(); i++) {
            if (((itemsRotate.get(i) - offsetRotate <= maxRemoveDegree)
                    && (itemsRotate.get(i) - offsetRotate >= minRemoveDegree)) && !itemAttached.get(i)) {
                View scrap = recycler.getViewForPosition(i);
                measureChildWithMargins(scrap, 0, 0);
                if (oritention == mScrollLeft)
                    addView(scrap, 0);
                else
                    addView(scrap);
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        float theta = dx / mDistanceRatio; // the angle every item will rotate for each dx
        float targetRotate = offsetRotate + theta;

        //handle the boundary
        if (targetRotate < 0) {
            willScroll = (int) (-offsetRotate * mDistanceRatio);
        } else if (targetRotate > getMaxOffsetDegree()) {
            willScroll = (int) ((getMaxOffsetDegree() - offsetRotate) * mDistanceRatio);
        }
        theta = willScroll / mDistanceRatio;
        ...
        if (dx < 0)
            layoutItems(recycler, state, mScrollLeft);
        else
            layoutItems(recycler, state, mScrollRight);
        return willScroll;
    }
    ...
    private float getMaxOffsetDegree() {
        return (getItemCount() - 1) * (float) intervalAngle;
    }
    ...
    public void scrollToPosition(int position) {
        ...
        float targetRotate = position * (float) intervalAngle;
    ...
    public int getOffsetCenterView() {
        return (int) ((getCurrentPosition() * intervalAngle - offsetRotate) * mDistanceRatio);
    }
